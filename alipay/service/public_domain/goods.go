...
type GoodsPromotionCaller interface {
	flow.GoodsPromotion
	UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler)
}

type GoodsPromotionService service.Service
...
func NewGoodsPromotionCaller(c *alipay.Client) GoodsPromotionCaller {
	// do some implantation
	caller := &GoodsPromotionService{Client: c}
	// 设置本产品相关接口默认参数校验规则
...
func (c GoodsPromotionService) UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler) {
	if c.RulersMap == nil {
...
func (c GoodsPromotionService) payloadChecker() paypay.PayloadRuler {
...
func (c GoodsPromotionService) setDefaultPayloadCheckRuler() map[string][]paypay.Ruler {
