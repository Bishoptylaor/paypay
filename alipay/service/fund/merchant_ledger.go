...
type MerchantLedgerCaller interface {
	flow.MerchantLedger
	UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler)
}

type MerchantLedgerService service.Service
...
func NewMerchantLedgerCaller(c *alipay.Client) MerchantLedgerCaller {
	// do some implantation
	caller := &MerchantLedgerService{Client: c}
	// 设置本产品相关接口默认参数校验规则
...
func (c MerchantLedgerService) UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler) {
	if c.RulersMap == nil {
...
func (c MerchantLedgerService) payloadChecker() paypay.PayloadRuler {
...
func (c MerchantLedgerService) setDefaultPayloadCheckRuler() map[string][]paypay.Ruler {
