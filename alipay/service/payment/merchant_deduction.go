...
type MerchantDeductionCaller interface {
	flow.MerchantDeduction
	UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler)
}

type MerchantDeductionService service.Service
...
func NewMerchantDeductionCaller(c *alipay.Client) MerchantDeductionCaller {
	// do some implantation
	caller := MerchantDeductionService{Client: c}
	// 设置本产品相关接口默认参数校验规则
...
func (c MerchantDeductionService) UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler) {
	if c.RulersMap == nil {
...
func (c MerchantDeductionService) payloadChecker() paypay.PayloadRuler {
...
func (c MerchantDeductionService) setDefaultPayloadCheckRuler() map[string][]paypay.Ruler {
