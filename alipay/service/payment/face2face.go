...
type Face2FacePayCaller interface {
	flow.Face2FacePay
	UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler)
}

type Face2FaceService service.Service
...
func NewFace2FaceCaller(c *alipay.Client) Face2FacePayCaller {
	// do some implantation
	caller := Face2FaceService{Client: c}
	// 设置本产品相关接口默认参数校验规则
...
func (c Face2FaceService) UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler) {
	if c.RulersMap == nil {
...
func (c Face2FaceService) payloadChecker() paypay.PayloadRuler {
...
func (c Face2FaceService) setDefaultPayloadCheckRuler() map[string][]paypay.Ruler {
