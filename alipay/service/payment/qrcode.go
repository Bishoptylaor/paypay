...
type QrcodeCaller interface {
	flow.QrcodePay
	UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler)
}

type QrcodeService service.Service
...
func NewQrcodeCaller(c *alipay.Client) QrcodeCaller {
	// do some implantation
	caller := &QrcodeService{Client: c}
	// 设置本产品相关接口默认参数校验规则
...
func (c QrcodeService) UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler) {
	if c.RulersMap == nil {
...
func (c QrcodeService) payloadChecker() paypay.PayloadRuler {
...
func (c QrcodeService) setDefaultPayloadCheckRuler() map[string][]paypay.Ruler {
