...
type PCPageCaller interface {
	flow.PcPagePay
	UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler)
}

type PCPageService service.Service
...
func NewPCPageCaller(c *alipay.Client) PCPageCaller {
	// do some implantation
	caller := &PCPageService{Client: c}
	// 设置本产品相关接口默认参数校验规则
	caller.RulersMap = caller.setDefaultPayloadCheckRuler()
	c.Use(alipay.Checker(caller.payloadChecker()))
	c.Use(alipay.PayloadPreSetter(map[string][]paypay.PayloadPreSetter{
...
func (c PCPageService) UseCustomPayloadCheckRuler(custom map[string][]paypay.Ruler) {
	if c.RulersMap == nil {
...
func (c PCPageService) payloadChecker() paypay.PayloadRuler {
...
func (c PCPageService) setDefaultPayloadCheckRuler() map[string][]paypay.Ruler {
